...
from models.models import HorarioClase
from seting.database import get_session
from seting.ResponseDTO import ResponseDTO

router = APIRouter(
    prefix="/horario_clases",
    tags=["Horario_Clases"],
    #dependencies=[Depends(get_current_user)]
)

@router.post("/guardar")
def create(item: HorarioClase, session: Session = Depends(get_session)):
    session.add(item)
    session.commit()
