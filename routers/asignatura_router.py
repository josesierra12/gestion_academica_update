...
from models.models import Asignatura
...
@router.post("/guardar")
def create(item: Asignatura, session: Session = Depends(get_session)):
    session.add(item)
    session.commit()
